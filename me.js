    ...
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          ...
          mediaType = (object && object.media_type) || (object && object.first_air_date ? "tv" : "movie");
          _object$card = object && object.card;
          tid = (_object$card && _object$card.imdb_id) || mediaType + "-" + object.id;

          settings = {
            url: "https://iboouvfabveldaehdnia.supabase.co/functions/v1/watchmode-proxy?tid=" +
                 tid +
                 "&apiKey=8e8jC2yZ5B3VWVJIwhk26VAA1fbUr1WmsDQsymjF" +
                 "&tmdbId=" + mediaType + "-" + object.id,
            method: "GET",
            timeout: 0
          };
              _context.next = 6;
...
Lampa.Listener.follow("full", function (e) {
  ...
  if (e.type == "complite") {
    var settings = {
      url: "https://api.mdblist.com/tmdb/"
        .concat(cardData.method == 'tv' ? 'show' : cardData.method, "/")
        .concat(cardData.id, "?apikey=kzql2cu8fytn86l2rk1frjppb"), // <-- hardcoded only
      method: "GET",
      timeout: 0
    };
